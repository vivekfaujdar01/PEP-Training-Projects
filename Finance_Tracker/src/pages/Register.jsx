    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-[#edfaf6] dark:bg-[#0a0f1e] transition-colors duration-300" style={{ backgroundImage: "radial-gradient(ellipse at 60% 20%, rgba(13,148,136,0.12) 0%, transparent 60%)" }}>
      <div className="w-full max-w-sm rounded-xl p-8 bg-white dark:bg-[#0d1530] border border-teal-100 dark:border-cyan-900/40 shadow-[0_4px_12px_rgba(0,0,0,0.05)] dark:shadow-[0_4px_24px_rgba(0,0,0,0.5)] animate-slide-up">
        ...
        <div className="text-center mb-7">
          ...
          <p className="text-xs text-teal-500/70 dark:text-cyan-600 mt-1">Create your free account</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-1.5">
            ...
            <input className={inputCls} name="password" type="password" value={form.password} onChange={handleChange} placeholder="Min. 6 characters" required />
          </div>
          <div className="flex flex-col gap-1.5">
            <label className={labelCls}>Confirm Password</label>
            <input className={inputCls} name="confirm" type="password" value={form.confirm} onChange={handleChange} placeholder="Repeat password" required />
          </div>
          <button type="submit" disabled={loading} className="mt-1 w-full py-2.5 rounded-lg text-sm font-bold text-white bg-linear-to-r from-teal-400 to-cyan-500 dark:from-cyan-400 dark:to-sky-500 hover:opacity-90 hover:-translate-y-0.5 disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 shadow-md cursor-pointer">{loading ? "Creating account..." : "Create Account"}</button>
        </form>
